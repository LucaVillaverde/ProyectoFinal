...
cron.schedule("*/1 * * * *", () => {
  //Tarea ejecutada cada 1 minuto
  db.run(
    "UPDATE Tokens SET cookieToken = 0, created_at = 0 WHERE julianday('now') - julianday(created_at) >= 1",
    (err) => {
      if (err) {
        ...
        if (!fs.existsSync(imagenRespaldo)) {
          fs.copyFile(imagenOriginal, imagenRespaldo, (err) => {
            if (err) {
              ...
              console.log(
                `Archivo ${imagen} copiado con éxito a ${carpetaRespaldo}`
              );
            }
          });
        } else {
...
app.get("/api/protection", (req, res) => {
  ...
  if (!tokenH || !id_user) {
    ...
    try {
      db.get(
        "SELECT cookieToken FROM Tokens WHERE id_user = ?",
        [id_user],
        async (err, row) => {
          if (err) {
            return res
          ...
          if (!row) {
            return res
              .status(404)
              .json({ message: "No existes en el sistema." });
          } else {
            const match = await bcrypt.compare(row.cookieToken, tokenH);
            if (match) {
              return res
                .status(200)
                .json({ message: "Token verificado correctamente." });
            } else {
  ...
  try {
    db.get("SELECT username FROM Users WHERE id_user = ?", [id], (err, row) => {
      ...
      if (!row) {
        return res
          .status(404)
          .json({ message: "No se han encontrado los usuarios" });
      }
    });
  } catch (err) {
    return res.status();
  ...
  try {
    ...
    db.get(
      ...
      async (err, row) => {
        if (err) {
          return res.status(500).json({
            message: `Error interno del servidor al verificar el valor de cookieToken para el usuario: ${usernameNH}.`,
          });
        }
        if (!row) {
        ...
        if (tokenDB === "0") {
              ...
              db.run(
                ...
                function (err) {
                  if (err) {
                    return res.status(500).json({
                      message:
                        "Error interno del servidor al actualizar el token.",
                    });
                  }
          ...
          return res.status(403).json({
            message: "No se puede iniciar sesión, el token ya está en uso.",
          });
        }
      }
    );
...
app.post("/api/login", (req, res) => {
  ...
  if (!username || !password) {
    // Cambié '&&' a '||' para verificar que ambas variables son necesarias
    return res
      .status(400)
  ...
  db.get(passwordQuery, [nombreMinusculas], async (err, row) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error al buscar el usuario. 500" });
    }
    if (!row) {
    ...
    if (row) {
      ...
      if (matchP) {
        return res
          .status(200)
          .json({ message: "Se ha iniciado sesión con exito." });
      } else {
        return res
          .status(400)
...
app.post("/api/register", (req, res) => {
  ...
  if (!usernameR || !passwordR || !passwordRC) {
    return res
      .status(400)
      .json({ message: "Faltan el nombre de usuario o las contraseñas. 400" });
  }
  ...
  if (
    validacionUsuario.empiezaConEspacio ||
    validacionPassword.empiezaConEspacio
  ) {
    return res
      .status(400)
      .json({ message: "No pueden iniciar con un espacio. 400" });
  }
  if (
    validacionUsuario.terminaConEspacio ||
    validacionPassword.terminaConEspacio
  ) {
    return res
      .status(400)
      .json({ message: "No pueden terminar con un espacio. 400" });
  }
  if (
    validacionUsuario.espaciosContinuos ||
    validacionPassword.espaciosContinuos
  ) {
    return res
      .status(400)
      .json({ message: "No puede haber un espacio seguido de otro. 400" });
  }
  ...
  try {
    db.get(query, [nombreMinisculas], async (err, row) => {
      ...
      if (row) {
        return res
          .status(400)
          .json({ message: "El nombre de usuario ya está en uso. 400" });
      } else {
        try {
          ...
          db.run(
            ...
            function (err) {
              ...
              return res
                .status(201)
                .json({ message: "Usuario creado", userId: userId });
            }
          );
        } catch (hashError) {
          ...
          return res
            .status(500)
            .json({ message: "Error al procesar la contraseña. 500" });
        }
      }
    });
  } catch (error) {
...
app.post("/api/logout", async (req, res) => {
  ...
  if (id_user) {
    try {
      db.run(
        "UPDATE Tokens SET cookieToken = 0, created_at = 0 WHERE id_user = ?",
        [id_user],
        (err) => {
          if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error al borrar el token de la Base de Datos." });
    }
  } else {
    res.clearCookie("id_user");
    res.clearCookie("username");
  ...
  if (user) {
    ...
    try {
      db.run(deleteQueryTokensTable, [user], (err) => {
        if (err) {
          return res.status(500).json({
            message: "Error interno del server al eliminar el usuario.",
          });
        }
        ...
        db.run(deleteQueryUsersTable, [user], (err) => {
          if (err) {
            return res.status(500).json({
              message: "Error interno del server al eliminar al usuario.",
            });
          }
          return res.status(200).json({
            message: "Usuario eliminado con exito de la tabla Users y Tokens.",
          });
        });
      });
    } catch (err) {}
    ...
    return res
      .status(400)
      .json({ message: "No se ha indicado un usuario para borrar." });
  }
});
...
app.post("/api/checkeo", async (req, res) => {
  ...
  try {
    db.get(
      "SELECT cookieToken, username, created_at FROM Tokens WHERE id_user = ?",
      [idUserNav],
      async function (err, row) {
        if (err || !row) {
          ...
          return res.status(400).json({
            success: false,
            message:
              "Error al obtener los datos del usuario o no se encontraron",
          });
        }
        ...
        if (tiempoExpirado(row.created_at, tiempoActual)) {
          db.run(
            ...
            function (updateErr) {
              if (!updateErr) {
                ...
                return res.status(400).json({
                  success: false,
                  message: "Token expirado y eliminado",
                });
              }
            }
          );
          ...
          return res
            .status(200)
            .json({ success: true, message: "Tokens válidos" });
        } else {
          res.clearCookie("id_user");
          res.clearCookie("username");
          res.clearCookie("token");
  ...
  if (anchoBoolean === 1) {
    ...
    db.all(getQuery, [limit], (err, rows) => {
      if (err) {
        return res.status(500).json({
          message:
            "Ha ocurrido un error interno del servidor al intentar conseguir las recetas.",
          error: err,
        });
      } else if (!rows) {
        return res.status(404).json({
...
app.post("/api/recetas/filtradas", (req, res) => {
  ...
  if (arrayCategorias && arrayCategorias.length > 0) {
    const placeholders = arrayCategorias
      .map(() => "categories LIKE ?")
      .join(" AND ");
  ...
  db.get(countQuery, params.slice(0, params.length - 2), (err, countResult) => {
    if (err) {
      return res.status(500).json({
        message: "Error al obtener el total de recetas filtradas.",
        error: err.message,
      });
    }
    ...
    db.all(query, params, (err, rows) => {
      if (err) {
        return res.status(500).json({
          message: "Error al obtener las recetas filtradas.",
          error: err.message,
        });
      }
      if (rows.length === 0) {
        return res.status(404).json({
          message: "No se encontraron recetas para los filtros aplicados.",
        });
      }
      return res.status(200).json({
...
app.post("/api/recetas/personales", (req, res) => {
  ...
  db.get(countQuery, [usernameNH], (err, countResult) => {
    ...
    const getQuery =
      "SELECT * FROM Recipe WHERE username = ? ORDER BY id_recipe DESC LIMIT ? OFFSET ?";
    db.all(getQuery, [usernameNH, limit, offset], (err, rows) => {
      if (err) {
        return res
          .status(500)
          .json({ message: "Error al determinar los datos de las recetas." });
      }
      if (!rows) {
...
const resizeImage = (req, res, next) => {
  ...
  try {
    db.get(
      "SELECT recipe_name FROM Recipe WHERE username = ? AND recipe_name = ?",
      [username, recipeName],
      async (err, row) => {
        if (err) {
          console.error(
            "Error al buscar receta en la base de datos:",
            err.message
          );
          return res
            .status(500)
            .json({ message: "Error al buscar recetas", error: err.message });
        }

        if (row) {
          return res
            .status(400)
            .json({ message: "Ya existe una receta con ese nombre." });
        } else {
          try {
            ...
            if (
              ...
              return res.status(400).json({
                message: `Las imágenes deben tener al menos 1280px de ancho y 720px de alto. Actual: ${metadata.width}x${metadata.height}`,
              });
            }
            ...
            return res
              .status(500)
              .json({ error: "Error al procesar la imagen" });
          }
        }
      }
    );
    ...
    console.error(
      "Error al procesar la imagen o al buscar receta:",
      error.message
    );
...
app.post(
  ...
  (req, res) => {
    ...
    db.all(checkCategoriesQuery, categoriesArray, (err, rows) => {
      if (err) {
        return res.status(500).json({
          message: "Error al verificar las categorías",
          error: err.message,
        });
      }

      const existingCategories = rows.map((row) => row.category);
      const missingCategories = categoriesArray.filter(
      ...
      db.run(insertQuery, values, function (err) {
        if (err) {
          return res
            .status(500)
            .json({ message: "Error al insertar receta", error: err.message });
        }
        return res.status(201).json({
...
const resizeImageEdited = async (req, res, next) => {
  ...
  db.get(
    "SELECT recipe_name, image FROM Recipe WHERE username = ? AND id_recipe = ?",
    [username, id_recipe],
    async (err, row) => {
      if (err) {
      ...
      if (req.file) {
        ...
        if (metadata.width < targetWidth || metadata.height < targetHeight) {
          return res.status(400).json({
            message:
              "Las imágenes deben tener al menos 1280px de ancho y 720px de alto.",
          });
        }
        ...
        if (fs.existsSync(imagenPath)) {
          ...
          req.file = {
            path: path.join("uploads", newFileName).replace(/\\/g, "/"),
          };
        }
        return next(); // Continuar al siguiente middleware
      } else {
    ...
    try {
      ...
      db.get(getImagePathQuery, [id_recipe], async (err, row) => {
        if (err) {
          return res
            .status(500)
            .json({ message: "Error al obtener la imagen actual." });
        }
        if (!row) {
          return res.status(404).json({ message: "Receta no encontrada." });
        }
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              return res
                .status(500)
                .json({ message: "Error al actualizar la receta." });
            }
            return res
              .status(200)
              .json({ message: "Receta actualizada con éxito." });
          }
        );
      });
    } catch (error) {
...
app.post("/api/eliminarReceta", (req, res) => {
  ...
  db.get(getPasswordFromUsers, [id_user], async (err, userRow) => {
    if (err) {
      return res
        .status(500)
    ...
    const matchP = await bcrypt.compare(contraseña, userRow.password);
    ...
    db.get(getRecipeImageQuery, [id_recipe], (err, row) => {
      ...
      if (!row || !row.image || row.image === "/assets/imagenDefault.webp" || row.image === "../src/assets/imagenDefault.webp") {
        ...
        db.run(deleteQueryRecipeTable, [id_recipe], (err) => {
          ...
          return res
            .status(200)
            .json({ message: "Se eliminó correctamente la receta." });
        });
      } else {
        ...
        if (fs.existsSync(imagenPath)) {
          ...
          console.log(
            "No se encontró la imagen en el sistema de archivos:",
            imagenPath
